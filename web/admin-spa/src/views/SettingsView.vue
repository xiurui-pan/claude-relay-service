            ...
            >
              <div class="flex items-center justify-between">
                <div>
                  <div class="flex items-center">
                    <div
                      class="mr-3 flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-emerald-500 to-cyan-600 text-white shadow-lg"
                    >
                ...
                <div class="flex items-center gap-3">
                  <span
                    ...
                    :class="
                      claudeConfig.openaiAdaptivePriorityEnabled
                  ...
                  <label class="relative inline-flex cursor-pointer items-center">
                    <input
                      v-model="claudeConfig.openaiAdaptivePriorityEnabled"
                      class="peer sr-only"
                      type="checkbox"
                      @change="saveClaudeConfig"
                    />
                ...
                <div class="flex">
                  ...
                  <div class="ml-3">
                    ...
                    <p class="mt-1 text-sm text-emerald-700 dark:text-emerald-300">
                      自动模式会使用 `codexUsage` 快照，若数据缺失或过期会自动回退手动 priority。
                    </p>
                  </div>
                </div>
              </div>

              <div v-if="claudeConfig.openaiAdaptivePriorityEnabled" class="mt-6 space-y-4">
                <div>
                  <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                    Codex 快照最大有效期（分钟）
                  </label>
                  <input
                    v-model.number="claudeConfig.openaiAdaptiveCodexUsageMaxAgeMinutes"
                  ...
                  />
                  <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                    超过该时长的使用快照会被视为过期，回退手动 priority
                  </p>
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                    周限额权重（secondaryWeight）
                  </label>
                  <input
                    v-model.number="claudeConfig.openaiAdaptiveSecondaryWeight"
                    class="mt-1 block w-full max-w-xs rounded-lg border border-gray-300 bg-white px-3 py-2 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 dark:border-gray-500 dark:bg-gray-700 dark:text-white sm:text-sm"
                    max="1"
                    min="0"
                    step="0.01"
                ...
                <div>
                  <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                    重置时间权重（resetTimeWeight）
                  </label>
                  <input
                    v-model.number="claudeConfig.openaiAdaptiveResetTimeWeight"
                    class="mt-1 block w-full max-w-xs rounded-lg border border-gray-300 bg-white px-3 py-2 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 dark:border-gray-500 dark:bg-gray-700 dark:text-white sm:text-sm"
                    max="1"
                    min="0"
                    step="0.01"
                ...
                <div>
                  <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                    手动优先级融合权重（manualPriorityWeight）
                  </label>
                  <input
                    v-model.number="claudeConfig.openaiAdaptiveManualPriorityWeight"
                    class="mt-1 block w-full max-w-xs rounded-lg border border-gray-300 bg-white px-3 py-2 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 dark:border-gray-500 dark:bg-gray-700 dark:text-white sm:text-sm"
                    max="1"
                    min="0"
                    step="0.01"
                ...
                <div>
                  <label class="inline-flex cursor-pointer items-center">
                    ...
                    ></div>
                    <span class="ml-3 text-sm font-medium text-gray-900 dark:text-gray-300">
                      同时对 OpenAI-Responses 账户启用自适应评分
                    </span>
                  </label>
                </div>
              </div>
...
const claudeConfig = ref({
  ...
  openaiAdaptivePriorityEnabled: true,
  openaiAdaptiveIncludeResponses: false,
  openaiAdaptiveCodexUsageMaxAgeMinutes: 720,
  ...
  try {
    ...
    if (response.success && isMounted.value) {
      claudeConfig.value = {
        ...
        openaiAdaptivePriorityEnabled: response.config?.openaiAdaptivePriorityEnabled ?? true,
        openaiAdaptiveIncludeResponses: response.config?.openaiAdaptiveIncludeResponses ?? false,
        openaiAdaptiveCodexUsageMaxAgeMinutes:
          response.config?.openaiAdaptiveCodexUsageMaxAgeMinutes ?? 720,
        openaiAdaptiveSecondaryWeight: response.config?.openaiAdaptiveSecondaryWeight ?? 0.45,
...
const saveClaudeConfig = async () => {
  ...
  try {
    const payload = {
      ...
      openaiAdaptivePriorityEnabled: claudeConfig.value.openaiAdaptivePriorityEnabled,
      openaiAdaptiveIncludeResponses: claudeConfig.value.openaiAdaptiveIncludeResponses,
      openaiAdaptiveCodexUsageMaxAgeMinutes:
